    ...
    def errorMonitor(self):
        ...
        for key in self.errorEXCODE:
            ...
            if len(lastesttasks_failed) > 3:
                ...
                for task in lastesttasks_failed:
                    ...
                    TABLE_CONTENT += '<tr align="center"><td> %s</td><td> %s</td><td> %s</td><td> %s</td><td> %s</td></tr>' % (task['PR'], task['commitId'], task['ciName'], key, task['targetUrl'])
                if key != 'NetWork Not Work':
                    print("exception_list: %s" %set(exception_list))
                    print(len(set(exception_list)))
        ...
        if TABLE_CONTENT_ALL != '':
            ...
            if lastalarms[0] == str(alarm_tasks_all):
                ...
                print(HTML_CONTENT)
                receiver = ['zhangchunle@baidu.com', 'zhouwei25@baidu.com', 'tianshuo03@baidu.com', 'v_duchun@baidu.com']
                title = '异常导致的连续失败'
