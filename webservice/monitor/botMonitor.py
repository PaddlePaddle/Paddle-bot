...
class PaddleBotMonitor(object):
    ...
    def updatePR(self, message):
        headers = {'authorization': "token ad4d5dea7caabf1137e6c3624183c7a64bf79c27", "accept": "application/vnd.github.v3+json", 'content-type': "application/json"}
        data = {"body": message}
    ...
    def monitor(self):
        ...
        print("91 first status: %s " %checkPRStatus)
        ...
        if re_checkCIStatus == None:
            time.sleep(10)
            re_checkCIStatus = self.checkCIStatus()
        print("91 second status: %s " %re_checkCIStatus)
        count = 1
        while count < 6 and re_checkCIStatus == checkPRStatus:
            ...
            time.sleep(30)
            re_checkCIStatus = self.checkCIStatus()
            print("restart smee  91 status: %s " %re_checkCIStatus)
            print("restart count: %s" %count)
            count += 1
        
        if count != 1: 
            restart_log = '%s Paddle-bot restart %s' %(time.strftime("%Y%m%d %H:%M:%S", time.localtime()), count) 
