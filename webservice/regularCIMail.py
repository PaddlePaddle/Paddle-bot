...
class WeeklyCIIndex():
    def __init__(self):
        ...
        self.keyIndexDict = {
            "成功任务的平均耗时/min": "xly_average_consum_time_success_commit",
            "成功任务的平均执行时间/min": "xly_average_exec_time_success_commit",
            "成功任务的平均排队时间/min": "xly_average_wait_time_success_commit",
            "所有任务的平均耗时/min": "xly_average_consum_time_all_commit",
            "所有任务的平均排队时间/min": "xly_average_wait_time_all_commit",
            "所有任务的平均执行时间/min": "xly_average_exec_time_all_commit",
            "所有任务的平均耗时/min": "xly_average_consum_time_all_commit",
            "所有任务的平均排队时间/min": "xly_average_wait_time_all_commit",
            "所有任务的平均执行时间/min": "xly_average_exec_time_all_commit",
            "所有任务的平均编译时间/min": "xly_buildTime_all_commit",
            "所有任务的平均单测时间/min": "xly_testCaseTime_total_all_commit",
    ...
    def getWeeklyCIIndex(self, startTime, endTime):
        ...
        for ci in self.requiredCIName:
            ...
            #所有commit执行时间
            average_exec_time_all_commit_query_stat = "select mean(execTime_total)/60 from paddle_ci_status where %s and commit_createTime > %s and commit_createTime < %s" % (
            ...
            #成功commit执行时间
            average_exec_time_success_commit_query_stat = "select mean(execTime_total)/60 from paddle_ci_status where %s and status='success' and EXCODE=0 and commit_createTime > %s and commit_createTime < %s" % (
                queryCIName, startTime_stamp, endTime_stamp)
    ...
    def getUserPerceptionIndexAndRerunByTestsFailed(self, ciIndex_thisWeek,
        ...
        3. 成功commit从提交到返回的平均耗时
        4. 所有commit从提交到返回的最长耗时CI及大小
        5. 成功commit从提交到返回的最长耗时CI及大小
        5. 单测随机挂引起的rerun率
        6. 平均失败率最大CI
        表2: 由于单测随机挂RERUN占比
        """
        ...
        #各个repo的commit的数目
        for repo in self.repo:
            UserPerceptionIndexContent += "<tr align=center><td bgcolor='#d0d0d0'>%s commit任务/个</td>" % repo.split(
                '/')[1]
            key = '%s_commitCount' % repo.split('/')[1]
            ...
            if float(DIFF_RATE) > 50:  #比率大于50在显示颜色
                ...
                UserPerceptionIndexContent += "<td>%s(↑ %s" % (thisWeekCommit,
                                                               DIFF_RATE)
            UserPerceptionIndexContent += "%)</td></tr>"

        #所有commit从提交到返回结果平均耗时/
        UserPerceptionIndexContent += "<tr align=center><td bgcolor='#d0d0d0'>所有commit任务从提交到返回结果平均耗时/min</td>"
        ...
        #成功commit从提交到返回结果平均耗时
        UserPerceptionIndexContent += "<tr align=center><td bgcolor='#d0d0d0'>成功commit任务从提交到返回结果平均耗时/min</td>"
        thisweek_max_consumtime_success_commit = 0
        lastweek_max_consumtime_success_commit = 0
        for ci in self.requiredCIName:
            key = '%s_average_consum_time_success_commit' % ci
            if float(ciIndex_thisWeek[
                    key]) > thisweek_max_consumtime_success_commit:
                thisweek_max_consumtime_success_commit = float(
                    ciIndex_thisWeek[key])
                thisweek_max_consumtime_ci = ci
            if float(ciIndex_lastWeek[
                    key]) > lastweek_max_consumtime_success_commit:
                lastweek_max_consumtime_success_commit = float(
                    ciIndex_lastWeek[key])
                lastweek_max_consumtime_ci = ci
        DIFF_RATE = "%.2f" % float(
            abs(thisweek_max_consumtime_success_commit -
                lastweek_max_consumtime_success_commit) /
            lastweek_max_consumtime_success_commit * 100)
        if float(DIFF_RATE) > 20:
            UserPerceptionIndexContent += "%s%s(↑ %s" % (
                rise_red_color, thisweek_max_consumtime_success_commit,
                DIFF_RATE
            ) if thisweek_max_consumtime_success_commit - lastweek_max_consumtime_success_commit > 0 else "%s%s(↓ %s" % (
                decline_green_color, thisweek_max_consumtime_success_commit,
                DIFF_RATE)
        else:
            UserPerceptionIndexContent += "<td>%s(↑ %s" % (
                thisweek_max_consumtime_success_commit, DIFF_RATE)
        UserPerceptionIndexContent += "%)</td></tr>"

        #所有commit从提交到返回的最长耗时CI及大小
        UserPerceptionIndexContent += "<tr align=center><td bgcolor='#d0d0d0'>所有commit任务从提交到返回结果最长耗时/min</td>"
        ...
        #成功commit从提交到返回的最长耗时CI及大小
        UserPerceptionIndexContent += "<tr align=center><td bgcolor='#d0d0d0'>成功commit任务从提交到返回结果最长耗时/min</td>"
        thisweek_longest_time_success_commit = float(ciIndex_thisWeek[
            'LongestTime_success_commit'])
        lastweek_longest_time_success_commit = float(ciIndex_lastWeek[
            'LongestTime_success_commit'])
        DIFF_RATE = "%.2f" % float(
            abs(thisweek_longest_time_success_commit -
                lastweek_longest_time_success_commit) /
            lastweek_longest_time_success_commit * 100)
        if float(DIFF_RATE) > 20:
            UserPerceptionIndexContent += "%s%s(↑ %s" % (
                rise_red_color, thisweek_longest_time_success_commit, DIFF_RATE
            ) if thisweek_longest_time_success_commit - lastweek_longest_time_success_commit > 0 else "%s%s(↓ %s" % (
                decline_green_color, thisweek_longest_time_success_commit,
                DIFF_RATE)
        else:
            UserPerceptionIndexContent += "<td>%s(↑ %s" % (
                thisweek_longest_time_success_commit, DIFF_RATE)
        UserPerceptionIndexContent += "%)"
        UserPerceptionIndexContent += "[%s %s]</td></tr>" % (
            ciIndex_thisWeek['LongestTime_success_commit_CINAME'],
            ciIndex_thisWeek['LongestTime_success_commit_PR'])
    ...
    def getInternalDetailIndex(self, ciIndex_thisWeek, ciIndex_lastWeek=0):
        ...
        ci_index_dic = {"00成功任务的平均耗时/min": "ci_average_consum_time_success_commit", "01成功任务的平均排队时间/min": "ci_average_wait_time_success_commit", \
            "02成功任务的平均执行时间/min": "ci_average_exec_time_success_commit", "05所有任务的平均耗时/min": "ci_average_consum_time_all_commit", \
            "06所有任务的平均排队时间/min": "ci_average_wait_time_all_commit", "07所有任务的平均执行时间/min": "ci_average_exec_time_all_commit", \
...
def sendMail(startTime, endTime, Content):
    ...
    mail.set_receivers(['xxx-all@xx.com'])
