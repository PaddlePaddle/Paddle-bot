...
class rerunServer():
    ...
    def rerunCI(self, ciName, target_url):
        ...
        res_status_code = xlyOpenApiRequest().get_method(rerun_url, param=query_param, headers=headers).status_code
        if res_status_code == 200:
            print('%s_%s rerun success!' %(ciName, target_url))
            logger.info('%s_%s rerun success!' %(ciName, target_url))
        else:
            print('%s_%s rerun failed: %s' %(ciName, target_url, res_status_code))
            logger.error('%s_%s rerun failed: %s' %(ciName, target_url, res_status_code))
    ...
    async def rerun_handler(self, request):
        ...
        logger.info('%s %s rerun ' %(PR, commit))
        '''
...
if __name__ == "__main__":
    ...
    app.router.add_get('/{user}/{repo}/{PR}/{commit}', rerunServer().rerun_handler)
