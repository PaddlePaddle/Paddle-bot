...
async def overdueList(types, url, gh):
    ...
    lastYear = str(today - datetime.timedelta(days=90))
    overduelist = []
    while (url != None):
...
async def main(user, repo):
    async with aiohttp.ClientSession() as session:
        ...
        try:
            ...
            gh = gh_aiohttp.GitHubAPI(session, user, oauth_token=access_token["token"])
            print(gh)
...
loop.run_until_complete(main('PaddlePaddle', 'PaddleOCR'))