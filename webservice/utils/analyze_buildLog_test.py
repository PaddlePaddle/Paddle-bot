...
from mail_163 import Mail
from handler_bak import xlyHandler
import pandas as pd
import os
import time
import datetime
...
def getIpipeBuildLog(sha, pipelineConfName, createTime, logUrl, typ=None):
    try:
        ...
        if typ == 'cpu':
            filename = "buildLog/%s_%s_%s_cpu.log" % (pipelineConfName, sha, createTime)
        elif typ == 'gpu':
            filename = "buildLog/%s_%s_%s_gpu.log" % (pipelineConfName, sha, createTime)
        else:
            filename = "buildLog/%s_%s_%s.log" % (pipelineConfName, sha, createTime)
        with open(filename, "wb") as f:
            f.write(r.content)
            f.close()


def get_indexttttt(index_dict, target_url):
    key_dict = {"ipipe_log_param_Build_Time":"buildTime",
        "ipipe_log_param_TestCases_Total_Time": "testCaseTime_total",
        "ipipe_log_param_1_Cards_TestCases_Total_Time": "testCaseTime_single",
        "ipipe_log_param_2_Cards_TestCases_Total_Time": "testCaseTime_multi",
        "ipipe_log_param_Exclusive_TestCases_Total_Time": "testCaseTime_exclusive",
        "ipipe_log_param_Exclusive_TestCases_Count": "testCaseCount_exclusive",
        "ipipe_log_param_1_Cards_TestCases_Count": "testCaseCount_single",
        "ipipe_log_param_2_Cards_TestCases_Count": "testCaseCount_multi",
        "ipipe_log_param_Exclusive_TestCases_Count": "testCaseCount_exclusive",
        "ipipe_log_param_PR_whl_Size": "WhlSize",
        "ipipe_log_param_Build_Size": "buildSize",
        "ipipe_log_param_Ccache_Hit_Rate": "ccacheRate",
        "ipipe_log_param_Paddle_Inference_Size": "fluidInferenceSize",
        "ipipe_log_param_Test_Fluid_Lib_Total_Time": "testFluidLibTime",
        "ipipe_log_param_Mac_TestCases_Time": "testCaseTime_total",
        "ipipe_log_param_Windows_Build_Time": "buildTime",
        "ipipe_log_param_Windows_Build_Cache": "buildCache",
        "ipipe_log_param_Windows_Paddle_Inference_Size": "fluidInferenceSize",
        "ipipe_log_param_Windows_PR_whl_Size": "WhlSize",
        "ipipe_log_param_Windows_TestCases_Total_Time": "testCaseTime_total",
        "ipipe_log_param_Windows_1_card_TestCases_Total_Time": "testCaseTime_single",
        "ipipe_log_param_Clcache_Hit_Rate": "clcacheRate"
    }
    not_request_xlylog_file = "buildLog/not_request_xlylog.txt"
    jobid = target_url.split('/')[-1]
    handler = xlyHandler()
    ci_index_res = handler.getCIindex(jobid)
    logger.info("xly return index status: %s" %ci_index_res)
    if len(ci_index_res.text) > 0:
        ci_result = ci_index_res.json()
        logger.info("xly return index json: %s" %ci_result)
    else:
        logger.info("xly return None: %s" %target_url)
        with open(not_request_xlylog_file, 'a+') as f:
            f.write(target_url+'\n')
        f.close()
    '''
    for index in ci_index_res:
        if index in ['ipipe_log_param_Windows_Build_Cache']:
            value = 1 if ci_index_res[index][0] == 'TRUE' else 0
        else:
            value = 0.0
            for v in ci_index_res[index]:
                value += float(re.findall(r"\d+\.?\d*",v)[0])
        index_dict[key_dict[index]] = value
    logger.info("test_index_dictttttt: %s" %index_dict)
    '''
...
class analysisBuildLog(object):
    def __init__(self, repo, sha, target_url):
        ...
        self.Paddle_cpu_gpu_separate_ci_tuple = tuple(localConfig.cf.get('CIIndexScope', 'Paddle_cpu_gpu_separate_ci').split(','))
        self.Paddle_container_ci = tuple(localConfig.cf.get('CIIndexScope', 'Paddle_container_ci').split(','))
        self.Other_sa_ci_tuple = tuple(localConfig.cf.get('CIIndexScope', 'Other_sa_ci').split(','))
        self.Other_container_ci = tuple(localConfig.cf.get('CIIndexScope', 'Other_container_ci').split(','))
        self.EXCODE_DICT = {'docker_build_failed': 64, 'clone_code_failed': 63, 'temporary_files_failed': 65, 'build_failed': 7, 'test_failed': 8, 'coverage_failed': 9, 'http_proxy_failed': 503, 'approve_failed': 6, 'code_style_failed': 4, 'code_conflict': 2, 'code_too_old': 15}
        self.SkipTestCi_tuple = tuple(localConfig.cf.get('CIIndexScope', 'Paddle_skip_test_ci').split(','))
        self.PRECISION_TEST_CI_tuple = tuple(localConfig.cf.get('CIIndexScope', 'Paddle_PRECISION_TEST').split(','))
        self.Paddle_testFailed_analysis_ci_tuple = tuple(localConfig.cf.get('CIIndexScope', 'Paddle_testFailed_analysis_ci').split(','))
        self.db = Database()
    ...
    def getIpipeBuildLog(self, pipelineConfName, createTime, logUrl, typ=None):
        try:
            ...
            if typ == 'cpu':
                filename = "buildLog/%s_%s_%s_cpu.log" % (pipelineConfName, self.sha, createTime)
            elif typ == 'gpu':
                filename = "buildLog/%s_%s_%s_gpu.log" % (pipelineConfName, self.sha, createTime)
            else:
                filename = "buildLog/%s_%s_%s.log" % (pipelineConfName, self.sha, createTime)
            with open(filename, "wb") as f:
                f.write(r.content)
                f.close()
    ...
    def getContainerCIIndex(self, stageBuildBeans):
        ...
        for stage in stageBuildBeans:
            ...
            if stageName == 'clone code':
                ...
                clone_code_startTime = int(str(jobGroupBuildBeans[0]['realJobBuild']['shellBuild']['startTime'])[:-3])
                clone_code_endTime = int(str(jobGroupBuildBeans[0]['realJobBuild']['shellBuild']['endTime'])[:-3])
                ...
                for job in jobGroupBuildBeans:
                    if job['jobName'] == 'Git-clone':
                        ...
                        clone_code_startTime = int(str(job['realJobBuild']['shellBuild']['startTime'])[:-3])
                        clone_code_endTime = int(str(job['realJobBuild']['shellBuild']['endTime'])[:-3])
                        ContainerCIIndex['clone_code_status'] = clone_code_status
                        ContainerCIIndex['clone_code_startTime'] = clone_code_startTime
                        ContainerCIIndex['clone_code_endTime'] = clone_code_endTime
                    elif job['jobName'] in ['构建镜像', 'build-docker-image']:
                        docker_build_status = job['status']
                        docker_build_startTime = int(str(job['realJobBuild']['startTime'])[:-3])
                        docker_build_endTime = int(str(job['realJobBuild']['endTime'])[:-3])
                        ContainerCIIndex['docker_build_status'] = docker_build_status
                        ContainerCIIndex['docker_build_startTime'] = docker_build_startTime
                        ContainerCIIndex['docker_build_endTime'] = docker_build_endTime
                    elif job['jobName'] == 'paddle-build':
                        cpu_build_status = job['status']
                        cpu_build_startTime = int(str(job['realJobBuild']['startTime'])[:-3]) if cpu_build_status != 'WAITTING' else 0
                        cpu_build_endTime = int(str(job['realJobBuild']['endTime'])[:-3]) if cpu_build_status != 'WAITTING' else 0
                        logParam = job['realJobBuild']['logUrl']
                        cpu_logUrl = localConfig.cf.get('ipipeConf', 'log_url') + logParam if cpu_build_status != 'WAITTING' else None
                        ContainerCIIndex['cpu_build_status'] = cpu_build_status
                        ContainerCIIndex['cpu_build_startTime'] = cpu_build_startTime
                        ContainerCIIndex['cpu_build_endTime'] = cpu_build_endTime
                        ContainerCIIndex['cpu_logUrl'] = cpu_logUrl
                    elif job['jobName'] == 'paddle-test':
                        gpu_test_status = job['status']
                        gpu_test_startTime = int(str(job['realJobBuild']['startTime'])[:-3]) if gpu_test_status != 'WAITTING' else 0
                        gpu_test_endTime = int(str(job['realJobBuild']['endTime'])[:-3]) if gpu_test_status != 'WAITTING' else 0
                        logParam = job['realJobBuild']['logUrl']
                        gpu_logUrl = localConfig.cf.get('ipipeConf', 'log_url') + logParam if gpu_test_status != 'WAITTING' else None
                        ContainerCIIndex['gpu_test_status'] = gpu_test_status
                        ContainerCIIndex['gpu_test_startTime'] = gpu_test_startTime
                        ContainerCIIndex['gpu_test_endTime'] = gpu_test_endTime
                        ContainerCIIndex['gpu_logUrl'] = gpu_logUrl
                    else:
                        paddle_build_status = job['status']
                        paddle_build_startTime = int(str(job['realJobBuild']['startTime'])[:-3]) if paddle_build_status != 'WAITTING' else 0
                        paddle_build_endTime = int(str(job['realJobBuild']['endTime'])[:-3]) if paddle_build_status != 'WAITTING' else 0
                        logParam = job['realJobBuild']['logUrl']
                        logUrl = localConfig.cf.get('ipipeConf', 'log_url') + logParam if paddle_build_status != 'WAITTING' else None
                        ContainerCIIndex['paddle_build_status'] = paddle_build_status
                        ContainerCIIndex['paddle_build_startTime'] = paddle_build_startTime
                        ContainerCIIndex['paddle_build_endTime'] = paddle_build_endTime
        ...
        for stage in stageBuildBeans:
            ...
            for job in jobGroupBuildBeans:
                if job['jobName'] == 'Git-clone':
                    ...
                    job_startTime = int(str(job['realJobBuild']['shellBuild']['startTime'])[:-3]) if job['status'] != 'WAITTING' else 0
                    job_endTime = int(str(job['realJobBuild']['shellBuild']['endTime'])[:-3]) if job['status'] != 'WAITTING' else 0
    ...
    def getBasicCIIndex(self, ciName):
        ...
        try:
            ...
            commit_createTime = int(str(res['pipelineBuildBean']['startTime'])[:-3]) #commit提交时间/rerun时间
            ...
            if self.repo != 'PaddlePaddle/Paddle':
                ...
                if res['pipelineConfName'].startswith(self.Paddle_container_ci):
                    CIIndex = self.getContainerCIIndex(stageBuildBeans)
                else:
                    CIIndex = self.getSaCIIndex(stageBuildBeans)
            if 'jobExecTime' in CIIndex: #sa任务
                if 'clone_code_status' in CIIndex:
                    waitTime_total = (CIIndex['clone_code_startTime'] - commit_createTime) + (CIIndex['job_startTime'] - CIIndex['clone_code_endTime'])
                    execTime_total = (CIIndex['jobExecTime']) + (CIIndex['clone_code_endTime'] - CIIndex['clone_code_startTime'])
                    basic_ci_index_dict['clone_code_startTime'] = CIIndex['clone_code_startTime']
                    basic_ci_index_dict['clone_code_endTime'] = CIIndex['clone_code_endTime']
                    basic_ci_index_dict['paddle_build_startTime'] = CIIndex['job_startTime']
                    basic_ci_index_dict['paddle_build_endTime'] = CIIndex['job_endTime']
                    ...
                    basic_ci_index_dict['paddle_build_startTime'] = CIIndex['job_startTime']
                    basic_ci_index_dict['paddle_build_endTime'] = CIIndex['job_endTime']
                    waitTime_total = CIIndex['job_startTime'] - commit_createTime
                    execTime_total = CIIndex['jobExecTime']
                if CIIndex['logUrl'] != None and self.repo in ['PaddlePaddle/Paddle']:
                    self.getIpipeBuildLog(res['pipelineConfName'], commit_createTime, CIIndex['logUrl'])
                if self.repo not in ['PaddlePaddle/Paddle']:
                    EXCODE = 0 if CIIndex['job_status'] == 'SUCC' else 1
                else:
                    if CIIndex['logUrl'] != None:
                        self.getIpipeBuildLog(res['pipelineConfName'], commit_createTime, CIIndex['logUrl'])
                        log_filename = "buildLog/%s_%s_%s.log" % (res['pipelineConfName'], self.sha, commit_createTime)
                        EXCODE = self.getExcode(res['pipelineConfName'], log_filename)
            elif 'cpu_build_status' in CIIndex: #cpu/gpu
                basic_ci_index_dict['clone_code_startTime'] = CIIndex['clone_code_startTime']
                basic_ci_index_dict['clone_code_endTime'] = CIIndex['clone_code_endTime']
                basic_ci_index_dict['docker_build_startTime'] = CIIndex['docker_build_startTime']
                basic_ci_index_dict['docker_build_endTime'] = CIIndex['docker_build_endTime']
                basic_ci_index_dict['paddle_build_startTime'] = CIIndex['cpu_build_startTime']
                basic_ci_index_dict['paddle_build_endTime'] = CIIndex['cpu_build_endTime']
                basic_ci_index_dict['paddle_gputest_startTime'] = CIIndex['gpu_test_startTime']
                basic_ci_index_dict['paddle_gputest_endTime'] = CIIndex['gpu_test_endTime']
                if CIIndex['cpu_logUrl'] != None and self.repo in ['PaddlePaddle/Paddle']:
                    self.getIpipeBuildLog(res['pipelineConfName'], commit_createTime, CIIndex['cpu_logUrl'], 'cpu')
                if CIIndex['gpu_logUrl'] != None and self.repo in ['PaddlePaddle/Paddle']:
                    self.getIpipeBuildLog(res['pipelineConfName'], commit_createTime, CIIndex['gpu_logUrl'], 'gpu')
                if CIIndex['clone_code_status'] == 'FAIL':
                    waitTime_total = CIIndex['clone_code_startTime'] - commit_createTime
                    execTime_total = CIIndex['clone_code_endTime'] -  CIIndex['clone_code_startTime']
                    EXCODE = self.EXCODE_DICT['clone_code_failed']
                elif CIIndex['docker_build_status'] == 'FAIL':
                    waitTime_total = (CIIndex['clone_code_startTime'] - commit_createTime) + (CIIndex['docker_build_startTime'] - CIIndex['clone_code_endTime'])
                    execTime_total = (CIIndex['clone_code_endTime'] -  CIIndex['clone_code_startTime']) + (CIIndex['docker_build_endTime'] - CIIndex['docker_build_startTime'])
                    EXCODE = self.EXCODE_DICT['docker_build_failed']
                elif CIIndex['cpu_build_status'] == 'FAIL':
                    waitTime_total = (CIIndex['clone_code_startTime'] - commit_createTime) + (CIIndex['docker_build_startTime'] - CIIndex['clone_code_endTime']) + (CIIndex['cpu_build_startTime'] - CIIndex['docker_build_endTime'])
                    execTime_total = (CIIndex['clone_code_endTime'] -  CIIndex['clone_code_startTime']) + (CIIndex['docker_build_endTime'] - CIIndex['docker_build_startTime']) + (CIIndex['cpu_build_endTime'] - CIIndex['cpu_build_startTime'])
                    log_filename = "buildLog/%s_%s_%s_cpu.log" % (res['pipelineConfName'], self.sha, commit_createTime)
                    EXCODE = self.getExcode(res['pipelineConfName'], log_filename)
                else:
                    waitTime_total = (CIIndex['clone_code_startTime'] - commit_createTime) + (CIIndex['docker_build_startTime'] - CIIndex['clone_code_endTime']) + (CIIndex['cpu_build_startTime'] - CIIndex['docker_build_endTime']) + (CIIndex['gpu_test_startTime'] - CIIndex['cpu_build_endTime'])
                    execTime_total = (CIIndex['gpu_test_endTime'] - CIIndex['gpu_test_startTime']) + (CIIndex['cpu_build_endTime'] - CIIndex['cpu_build_startTime']) + (CIIndex['docker_build_endTime'] - CIIndex['docker_build_startTime']) + (CIIndex['clone_code_endTime'] -  CIIndex['clone_code_startTime'])
                    log_filename = "buildLog/%s_%s_%s_gpu.log" % (res['pipelineConfName'], self.sha, commit_createTime)
                    EXCODE = self.getExcode(res['pipelineConfName'], log_filename)
            else: #container ci
                basic_ci_index_dict['docker_build_startTime'] = CIIndex['docker_build_startTime']
                basic_ci_index_dict['docker_build_endTime'] = CIIndex['docker_build_endTime']
                basic_ci_index_dict['paddle_build_startTime'] = CIIndex['paddle_build_startTime']
                basic_ci_index_dict['paddle_build_endTime'] = CIIndex['paddle_build_endTime']
                if self.repo not in ['PaddlePaddle/Paddle']:
                    waitTime_total = (CIIndex['docker_build_startTime'] - commit_createTime) + (CIIndex['paddle_build_startTime'] - CIIndex['docker_build_endTime']) if CIIndex['docker_build_status'] == 'SUCC' else CIIndex['docker_build_startTime'] - commit_createTime
                    execTime_total = (CIIndex['paddle_build_endTime'] - CIIndex['paddle_build_startTime']) + (CIIndex['docker_build_endTime'] - CIIndex['docker_build_startTime']) if CIIndex['docker_build_status'] == 'SUCC' else CIIndex['docker_build_endTime'] - CIIndex['docker_build_startTime']
                    EXCODE = 0 if CIIndex['paddle_build_status'] == 'SUCC' else 1
                else:
                    if CIIndex['logUrl'] != None:
                        self.getIpipeBuildLog(res['pipelineConfName'], commit_createTime, CIIndex['logUrl'])
                    if 'clone_code_status' in CIIndex:
                    ...
                    if CIIndex['docker_build_status'] == 'FAIL':
                        waitTime_total = (CIIndex['clone_code_startTime'] - commit_createTime) + (CIIndex['docker_build_startTime'] - CIIndex['clone_code_endTime']) if 'clone_code_status' in CIIndex else CIIndex['docker_build_startTime'] - commit_createTime
                        execTime_total = (CIIndex['clone_code_endTime'] -  CIIndex['clone_code_startTime']) + (CIIndex['docker_build_endTime'] - CIIndex['docker_build_startTime']) if 'clone_code_status' in CIIndex else CIIndex['docker_build_endTime'] - CIIndex['docker_build_startTime']
                        EXCODE = self.EXCODE_DICT['docker_build_failed']
                    else:
                        waitTime_total = (CIIndex['clone_code_startTime'] - commit_createTime) + (CIIndex['docker_build_startTime'] - CIIndex['clone_code_endTime']) + (CIIndex['paddle_build_startTime'] - CIIndex['docker_build_endTime']) if 'clone_code_status' in CIIndex else (CIIndex['docker_build_startTime'] - commit_createTime) + (CIIndex['paddle_build_startTime'] - CIIndex['docker_build_endTime'])
                        execTime_total = (CIIndex['paddle_build_endTime'] - CIIndex['paddle_build_startTime']) + (CIIndex['docker_build_endTime'] - CIIndex['docker_build_startTime']) + (CIIndex['clone_code_endTime'] -  CIIndex['clone_code_startTime']) if 'clone_code_status' in CIIndex else (CIIndex['docker_build_endTime'] - CIIndex['docker_build_startTime']) + (CIIndex['paddle_build_endTime'] - CIIndex['paddle_build_startTime'])
                        log_filename = "buildLog/%s_%s_%s.log" % (res['pipelineConfName'], self.sha, commit_createTime)
                        EXCODE = self.getExcode(res['pipelineConfName'], log_filename)
            basic_ci_index_dict['EXCODE'] = EXCODE
    ...
    def getDetailsCIIndex(self, basic_ci_index):
        ...
        detailed_ci_index_dict['execTime_total'] = basic_ci_index['execTime_total']
        detailed_ci_index_dict['waitTime_total'] = basic_ci_index['waitTime_total']
        ...
        if ciName.startswith(('PR-CI-APPROVAL', 'PR-CI-OP-benchmark', 'PR-CI-Model-benchmark')) or EXCODE in [1, 2, 7, 64, 503]:
            pass
        else:
            if ciName.startswith(self.Paddle_cpu_gpu_separate_ci_tuple):
                filename = '%s_%s_%s_cpu.log' % (ciName, commitId, basic_ci_index['commit_createTime'])
            else:
                filename = '%s_%s_%s.log' % (ciName, commitId, basic_ci_index['commit_createTime'])
            ...
            #收集ccache
            if ciName in ['PR-CI-Coverage', 'PR-CI-Py3', 'PR-CI-CPU-Py2', 'PR-CI-Inference', 'PR-CI-Mac', 'PR-CI-Mac-Python3']:
            ...
            if filename.startswith('PR-CI-Inference'):
                ...
                detailed_ci_index_dict['testFluidLibTime'] = float(testFluidLibTime)
                ...
                if filename.startswith('PR-CI-Coverage') or filename.startswith('PR-CI-Py3'):
                    if ciName.startswith(self.Paddle_cpu_gpu_separate_ci_tuple):
                        filename = '%s_%s_%s_gpu.log' % (ciName, commitId, basic_ci_index['commit_createTime'])
                        f = open('buildLog/%s' % filename, 'r')
                        data = f.read()
                    if 'in PRECISION_TEST' in data: #命中精致测试 只拿testCaseTime_total
                        ...
                        detailed_ci_index_dict['testCaseTime_total'] = testCaseTime_total
                    else:
                        ...
                        detailed_ci_index_dict['testCaseCount_multi'] = testCaseCount_multi
                        ...
                        detailed_ci_index_dict['testCaseCount_exclusive'] = testCaseCount_exclusive
                        ...
                        detailed_ci_index_dict['testCaseTime_multi'] = testCaseTime_multi
                        ...
                        detailed_ci_index_dict['testCaseTime_exclusive'] = testCaseTime_exclusive
                        if detailed_ci_index_dict['EXCODE'] == 8:
                            testCaseTime_total = detailed_ci_index_dict['testCaseTime_single'] + detailed_ci_index_dict['testCaseTime_multi'] + detailed_ci_index_dict['testCaseTime_exclusive']
                        else:
                            testCaseTime_total_strlist = data.split('TestCases Total Time:')
                            testCaseTime_total = 0
                            for item in testCaseTime_total_strlist[1:]:
                ...
                if ciName in ['PR-CI-Windows', 'PR-CI-Windows-OPENBLAS']:
                    buildCache_strlist = data.split('ipipe_log_param_Windows_Build_Cache: ', 2)
    ...
    def analyze_failed_cause(self, index_dict):
        ...
        analysis_ci_index['commit_createTime'] = index_dict['commit_createTime']
        ...
        if EXCODE == 0:
            isException = 0
            analysis_ci_index['description'] = 'document_fix' if index_dict['documentfix'] == 'True' else 'success'
            ...
            query_stat = "SELECT EXCODE,PR,commitId FROM paddle_ci_index WHERE ciName='%s' order by time desc limit 5" %index_dict['ciName']
            result = list(self.db.query(query_stat))
            if len(result) == 0:
                isException = 0
            else:
                last5tasks_buildfailed = [record['EXCODE'] for record in result[0] if record['EXCODE'] == 7]
                if len(last5tasks_buildfailed) < 3:
                    isException = 0
                else:
                    isException = 1
            if index_dict['ciName'].startswith(self.Paddle_cpu_gpu_separate_ci_tuple) and '2' not in index_dict['ciName']:
                analysis_ci_index['cpu_waitTime'] = (index_dict['docker_build_startTime'] - index_dict['clone_code_endTime']) + (index_dict['clone_code_startTime'] - index_dict['commit_createTime'])
            analysis_ci_index['description'] = 'build_failed'
        elif EXCODE == self.EXCODE_DICT['test_failed'] and index_dict['ciName'].startswith(self.Paddle_testFailed_analysis_ci_tuple): #单测失败原因
            ...
            if index_dict['ciName'].startswith(self.Paddle_cpu_gpu_separate_ci_tuple) and '2' not in index_dict['ciName']:
                analysis_ci_index['cpu_waitTime'] = (index_dict['docker_build_startTime'] - index_dict['clone_code_endTime']) + (index_dict['clone_code_startTime'] - index_dict['commit_createTime'])
                analysis_ci_index['gpu_waitTime'] = index_dict['paddle_gputest_startTime'] - index_dict['paddle_build_endTime']
            if index_dict['ciName'].startswith(self.Paddle_cpu_gpu_separate_ci_tuple):
                filename = '%s_%s_%s_gpu.log' % (index_dict['ciName'], index_dict['commitId'], index_dict['commit_createTime'])
            else:
                filename = '%s_%s_%s.log' % (index_dict['ciName'], index_dict['commitId'], index_dict['commit_createTime'])
            f = open('buildLog/%s' % filename, 'r')
            data = f.read()
            if index_dict['ciName'].startswith('PR-CI-Windows') and index_dict['ciName'] != 'PR-CI-Windows-Remain-BuildTest': #Mac/Windows
                ...
                testsfailed_strlist = data.rsplit("/",1)split('Summary Failed Tests...', 1)
            ...
            analysis_ci_index['description'] = 'Coverage Rate NOT Reach The Standard'
            isException = 0
        else:
        ...
        # 获取精准测试监控指标
        if index_dict['ciName'].startswith(self.PRECISION_TEST_CI_tuple) and EXCODE not in [self.EXCODE_DICT['clone_code_failed'], self.EXCODE_DICT['docker_build_failed'], self.EXCODE_DICT['build_failed'], self.EXCODE_DICT['http_proxy_failed'], self.EXCODE_DICT['code_conflict'], self.EXCODE_DICT['code_too_old']]:
            PRECISION_TEST = None
            ...
            if 'ipipe_log_param_PRECISION_TEST_Cases_count' in data:
                PRECISION_TEST_Cases_count = data.split('ipipe_log_param_PRECISION_TEST_Cases_count:', 1)
                PRECISION_TEST_Cases_count = int(PRECISION_TEST_Cases_count[1:][0].split('\n')[0].strip())
            if 'ipipe_log_param_PRECISION_TEST_Cases_ratio' in data:
                PRECISION_TEST_Cases_ratio = data.split('ipipe_log_param_PRECISION_TEST_Cases_ratio:', 1)
                PRECISION_TEST_Cases_ratio = round(float(PRECISION_TEST_Cases_ratio[1:][0].split('\n')[0].strip()), 2)
        ...
        if result == True:
            logger.info('%s insert paddle_ci_analysis success!' %analysis_ci_index)
        else:
    ...
    def send_utfailed_mail(self, alarm_ut_dict):
        ...
        if alarm_ut_dict == lastestfaileduts:
            ...
            for ut in alarm_ut_dict:
                for l in alarm_ut_dict[ut]:
                    ...
                    if ut not in lastestfaileduts:
                        TABLE_CONTENT += '<tr align="center" bgcolor="#b5c4b1"><td> %s</td><td> %s</td><td> %s</td><td> %s</td><td> %s</td></tr>' %(ut, pr, commit, ciname, ciurl)
                    else:
            ...
            receiver = ['zhangchunle@baidu.com', 'tianshuo03@baidu.com', 'v_duchun@baidu.com', 'xieyunshen@baidu.com', 'liuxudong04@baidu.com', 'luotao02@baidu.com']
            title = '[告警] CI单测挂了三次以上！'
            sendMail(receiver, title, HTML_CONTENT)

sha = 'c2fa3b666f76d3568244614c01ddd201a1b68465'
document_fix = 'false'
target_url = 'https://xly.bce.baidu.com/paddlepaddle/paddle/newipipe/detail/3130618/job/5612655'  #windows
# 'https://xly.bce.baidu.com/paddlepaddle/paddle/newipipe/detail/3107107/job/5516719' #build failed
# 'https://xly.bce.baidu.com/paddlepaddle/Paddle-Lite/newipipe/detail/3104104/job/5505839' #lite
# 'https://xly.bce.baidu.com/paddlepaddle/fluid-doc/newipipe/detail/3107664/job/5519245' #doc
# 
# 'https://xly.bce.baidu.com/paddlepaddle/paddle/newipipe/detail/3105985/job/5512211' #取消
# 'https://xly.bce.baidu.com/paddlepaddle/paddle/newipipe/detail/3105119/job/5509517'  #py3
#'https://xly.bce.baidu.com/paddlepaddle/paddle/newipipe/detail/3103473/job/5503576'
# 'https://xly.bce.baidu.com/paddlepaddle/paddle/newipipe/detail/3086856/job/5430385'
#getBasicCIIndex1('PaddlePaddle/Paddle', sha, document_fix, target_url)
a= analysisBuildLog('PaddlePaddle/Paddle', sha, target_url).getBasicCIIndex(document_fix)
analysisBuildLog('PaddlePaddle/Paddle', sha, target_url).getDetailsCIIndex(a)