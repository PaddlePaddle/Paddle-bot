...
def analyze_failed_cause(index_dict, target_url):
    ...
    PRECISION_TEST_CI = localConfig.cf.get('CIIndexScope', 'Paddle_PRECISION_TEST').split(',')
    isSkipTest = 0
    PRECISION_TEST = None
    ...
    if index_dict['ciName'] == SkipTestCi:
        f = open('buildLog/%s' % filename, 'r')
        data = f.read()
