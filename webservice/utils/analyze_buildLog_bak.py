        ...
        try:
            ...
            for job in jobGroupBuildBeans:
                ...
                if jobName in ['构建镜像', 'build-docker-image']:
                    ...
                    git_clone_status = job['status']
                    if git_clone_status == 'FAIL':
                        logParam = job['realJobBuild']['logUrl']
                        logUrl = localConfig.cf.get('ipipeConf', 'log_url') + logParam
                        break
...
def getIpipeBuildLog(sha, pipelineConfName, createTime, logUrl):
    ...
    if os.path.exists(filename):
        ...
        try:
            r = requests.get(logUrl, stream = True)
        except Exception as e:
            print("get log Error: %s" % e)
        else:
            with open("buildLog/%s_%s_%s.log" % (pipelineConfName, sha, createTime), "wb") as f:
                for chunk in r.iter_content(chunk_size = 1024):
                    if chunk:
                        f.write(chunk)
                f.close
            print("buildLog/%s_%s_%s.log" % (pipelineConfName, sha, createTime))
...
def analyze_failed_cause(index_dict, target_url):
    ...
    if EXCODE in [0, 4]: #2代码冲突，6需要approve, 4代码风格不符合
        ...
        if len(result) == 0:
            isException = 0
            print("isException: %s" %isException)
        else:
            ...
            for record in result[0]:
                if record['EXCODE'] == 7:
                    buildfailed_record = {}
                    buildfailed_record['PR'] = record['PR']
                    buildfailed_record['commitId'] = record['commitId']
                    last5tasks_buildfailed.append(buildfailed_record)
            '''
            if len(last5tasks_buildfailed) < 3:
                isException = 0
            else:
                isException = 1
            print("isExceptionxx: %s" %isException)
            
    elif EXCODE == 8 and index_dict['ciName'] not in ['PR-CI-OP-benchmark']: #单测失败原因
        ...
        if len(testsfailed_list) > 20:
            ...
            print('alarm_ut_dict_ult : %s' %alarm_ut_dict_ult)          
...
def send_buildfailed_mail(ciName, last5tasks_buildfailed):
    HTML_CONTENT = "<html> <head></head> <body>  <p>Hi, ALL:</p>  <p>最新的5个%s任务已经有3个以上都编译失败，请值班同学查看是否为编译连续挂。</p>" %(ciName)
    TABLE_CONTENT = '<table border="1" align="center"> <caption> <font size="3"><b>编译失败列表</b></font>  </caption> <tbody> <tr align="center"> <td bgcolor="#d0d0d0">PR</td> <td bgcolor="#d0d0d0">commitId</td> </tr> '
    for task in last5tasks_buildfailed:
        TABLE_CONTENT += '<tr align="center" bgcolor="#b5c4b1"><td> %s</td><td> %s</td></tr>' %(task['PR'], task['commitId'])
...
#target_url = 'https://xly.bce.baidu.com/paddlepaddle/paddle/newipipe/detail/2509981/job/3507334'


#analyze_failed_cause(index_dict, target_url)
repo = 'PaddlePaddle/Paddle'
target_url = 'https://xly.bce.baidu.com/paddlepaddle/paddle/newipipe/detail/2576099/job/3591393'
document_fix = False
sha = '8675f96ad5821f5cb4624a58117a8d03f71fcc63'
getBasicCIIndex(repo, sha, document_fix, target_url)
