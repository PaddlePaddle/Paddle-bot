...
def get_stage_url( target_url ):
    ...
    stage_url = local_config.cf.get( 'ipipeConf', 'stage_url' ) + pipeline_build_id 
    return stage_url
            ...
            for job in job_group_build_beans:
                ...
                if status == 'FAIL':
                    ...
                    if 'logUrl' in job[ 'realJobBuild' ]:
                        ...
                        logUrl = local_config.cf.get('ipipeConf', 'log_url') + logParam
                    else:
                    ...
                    log_name = 'stageBuildId-%d_jobId-%d_jobName-%s.log' % ( job[ 'stageBuildId' ], job[ 'id' ], job[ 'jobName' ] )
...
def remove_prefix_date( line ):
    result = re.match(r"(\d{4}-\d{1,2}-\d{1,2}\s\d{1,2}:\d{1,2}:\d{1,2})", line)
    if result != None:
