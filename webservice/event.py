                ...
                if state in ['success', 'failure', 'error']:
                    if state == 'success':
                        ...
                        await create_add_ci_failure_summary(
                            gh, context, repo, comment_url, ci_link, shortId, pr_num,
                            comment_list, commits_url)


async def create_add_ci_failure_summary(gh, context, repo, comment_url, ci_link,
                                        shortId, pr_num, comment_list,
                                        commits_url):
    """gradually find failed CI"""
    hyperlink_format = '<a href="{link}">{text}</a>'
    pr_link = "https://github.com/PaddlePaddle/" + repo + "/pull/" + str(pr_num)
    pr_hyperlink = hyperlink_format.format(link=pr_link, text="PR:" + str(pr_num))
